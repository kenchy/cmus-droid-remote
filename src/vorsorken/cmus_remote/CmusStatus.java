...
import vorsorken.cmus_remote.R;
...
public class CmusStatus {
	...
	public int getProgress() {
		// note: ProgressBar is set to max 1000 for finer increments
		try {
			int positionInt = Integer.parseInt(position);
			int durationInt = Integer.parseInt(duration);
			return (int) ((1000*positionInt)/(double)durationInt);
	...
	public String getRunTime() {
		...
		try {
			...
			runMin = positionInt / 60;
			runSec = positionInt % 60;;
			durMin = durationInt / 60;
			durSec = durationInt % 60;
		} catch(NumberFormatException e) {
			// no track playing
		}
		return String.format(runMin + ":%02d/" + durMin + ":%02d", runSec, durSec);
	}
}
